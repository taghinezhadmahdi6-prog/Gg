...
def build_txt_report(report_obj: dict, raw_fallback_text: str) -> bytes:
    ...
    for inv in invoices:
        ...
        for it in items:
            ...
            qty_fa = to_persian_digits(qty)
            up_fa = to_persian_digits(unit_price)
            tp_fa = to_persian_digits(total_price)
            lines.append(fmt_row([inv_no, name, qty_fa, up_fa, tp_fa]))
    lines.append("-" * 60)
...
def build_xlsx_report(report_obj: dict, raw_fallback_text: str) -> bytes:
    ...
    wb.save(bio)
    return bio.getvalue()
async def send_report_files(update: Update, context: ContextTypes.DEFAULT_TYPE, txt_bytes: bytes, xlsx_bytes: bytes, txt_name: str, xlsx_name: str):
...
async def process_with_clarifai(images, prompt):
    ...
    response = await clarifai_client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a helpful accounting assistant that extracts structured invoice data and returns valid JSON only."},
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "📁 هر وقت خواستید گزارش‌های قبلی را با دکمه مربوطه می‌فرستم.",
...
async def process_all_invoices(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    prompt_text = (
        "تو یک حسابدار دقیق هستی. این تصاویر فاکتور خرید هستند.\n"
        "فقط و فقط یک JSON معتبر برگردان (بدون متن اضافی).\n"
        "ساختار دقیق خروجی باید این باشد:\n"
        "{\n"
        '  "currency": "ریال",\n'
        '  "invoices": [\n'
        "    {\n"
        ...
        '  "notes": "هر توضیح لازم"\n'
        "}\n"
        "نکته: اگر بخشی ناخوانا بود مقدار را null بگذار."
    )
    result_text = ""
    source_used = ""
    try:
        logging.info(f"User {user_id}: Trying Google Gemini...")
    ...
    await update.message.reply_text(f"📊 گزارش نهایی آماده شد (توسط {source_used}).\n📎 فایل‌ها را هم ارسال می‌کنم...")
    await send_report_files(update, context, txt_bytes, xlsx_bytes, txt_name, xlsx_name)
    user_invoices[user_id] = []
...
if __name__ == '__main__':
    ...
    print("ربات با خروجی فایل (XLSX/TXT) و دکمه گزارش‌های قبلی روشن شد...")
